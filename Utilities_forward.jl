    mutable struct Parameters_Forward
    ...
    Parameters_Forward() = Parameters_Forward(8.3145, 9.8, 3.1556926e7, 1.602176565e-13, 6.022e23, 5.552729156131902e-10, 1.4203835667211993e-11, 9.845840632953767e-10, 1.551358953804712e-10, 4.933431890106372e-11, 4.0e24, 2.0e24, 6.371e6, 1.0e6, 1000.0, 800.0, 3500.0, 12300.0, 4.35e14, 2.58e13, 1718.8, 6416.1, 1.0e14, 1.0e13, 200.0, 2000.0, 3.5e13, 6.0e12, 1573.15, 4673.15, 2.0e13, 1.0e13, 100.0, 1000.0, 2.5e-5, 8.6e-7, 3773.15, 1.0e19, 0.2, 30.0, 1573.15, 1.0e21, 400000.0, 800000.0, 3.0, 1.2, 1.11, 1.0e23, 4.5e5, 0.0)

    # # Define parameters
    # p = Parameters_Forward() # Create struct
    ...
    # p.Tc_initial = 6416.1 # Initial core temperature (K).
...
## --- Calculate past mantle heat production model for a given time vector
    ...
    function mantle_heat_production(p::Parameters_Forward,composition::Dict,timevec)
...
## --- Model functions
    ...
    function coolingmodel_forward(p::Parameters_Forward, Ur, Hm, dt, timevec)
        ...
        Qm[1] = p.Qm_initial # Present mantle heat flux (W)
        Qc[1] = p.Qc_initial # Present core heat flux (W)
        Tm[1] = p.Tm_initial # Present mantle temperature (K)
        Tc[1] = p.Tc_initial # Present core temperature (K)
        ...
        # Scale mantle heat production for present-day Urey ratio
        Hm *= (Ur * 35.0E12 / (Hm[end] * p.Mm))
    ...
    function signed_forward_LL(Tm, timevec, TrelAve_time, TrelAve, TrelAve_sigma)
        Trel = linterp1s(timevec, Tm .- Tm[end], TrelAve_time)
    ...
    function coolingmodel_forward_LL(p::Parameters_Forward, Ur, Hm, dt, timevec, TrelAve_time, TrelAve, TrelAve_sigma)
        (Tm, Tc, Qm, Qc, dp) = coolingmodel_forward(p, Ur, Hmvec, dt, timevec)
        return signed_forward_LL(Tm, timevec, TrelAve_time, TrelAve, TrelAve_sigma)
    end

    # Return the residual for a given temperature curve
    function resid_forward(Tm, timevec, TrelAve_time, TrelAve, TrelAve_sigma)
        Trel = linterp1s(timevec, Tm .- Tm[end], TrelAve_time)
        return sum((Trel-TrelObs).^2 ./ (2 .* TrelObs_sigma.^2))
    end

    # Run model and calculate its residual
    function coolingmodel_forward_resid(p::Parameters_Forward, Ur, Hm, dt, timevec, TrelAve_time, TrelAve, TrelAve_sigma)
        (Tm, Tc, Qm, Qc, dp) = coolingmodel_forward(p, Ur, Hmvec, dt, timevec)
        return resid_forward(Tm, timevec, TrelAve_time, TrelAve, TrelAve_sigma)
    end

    function coolingmodel_forward_Newton_Ur(p::Parameters_Forward,nSteps,Hmvec,dt,timevec,TrelObs_time,TrelObs,TrelObs_sigma)
        Ur = 0.5
        dUr = 1E-9
        for i=1:nSteps-1
            ll = coolingmodel_forward_LL(p,Ur,Hmvec,dt,timevec,TrelObs_time,TrelObs,TrelObs_sigma)
            llp = coolingmodel_forward_LL(p,Ur+dUr,Hmvec,dt,timevec,TrelObs_time,TrelObs,TrelObs_sigma)
            dll_dUr = (llp-ll)/dUr
            Ur = Ur - ll/dll_dUr/2
        end
        return Ur
